...
The initial layout can be changed in Config. A valid MBR layout must contain 1–4 partitions and exactly one `*` remaining-space partition.

### Configuration profiles

Choose a named profile from Config's dropdown. **Save as…** opens a popup to name a new profile; **Save profile** updates the selection and acknowledges the save. **Remove** deletes a profile; there is no separate Rename action. The bottom **Save** commits all changes and shows a confirmation, while **Cancel** discards changes made in that Config session. Existing settings migrate to **My configuration**. Profiles include partitions, generated Windows Setup settings, compression, and the unsigned-driver setting; language/theme are app-wide and content paths are not saved in profiles.

Profile partitions and Windows Setup controls are greyed out while locked. Use the lock icon to edit them. **Apply profile** on the main screen reloads the selected partition layout; if content is attached, it asks before clearing those selections.

### Extracted Windows installer folders

Instead of an ISO, select a full extracted installer with **Add → Files / folders → Add Folder** on a fixed-size NTFS boot partition. The root must contain `setup.exe`, `bootmgr`, the BIOS/UEFI boot files, `sources/boot.wim`, and `sources/install.wim` or `sources/install.esd`. Choose the edition when prompted. Scripts and driver injection work with either source type. Ordinary support folders and split `install.swm` images do not qualify. Only one installer source is supported per USB.

Folder media is hashed, copied to local staging, serviced, and cached without modifying the backup. OneDrive-backed files are included and may need downloading. Linked files/folders are rejected; unavailable source files stop preparation before USB erasure. The prepared installer replaces the raw folder copy so an unserviced install image is not copied alongside it. An existing root answer file retains precedence and is preserved when scripts are integrated.

### Generated Windows Setup

**Protect installer USB from overwrite** is one optional checkbox, off by default. It adds all disk checks to generated XML: Disk 0 only, 200–4000 GiB inclusive, exactly one matching disk, and stop on disk-check errors. Target disk must be 0 while enabled. Existing partitions are allowed and will be erased during installation. Failed checks, missing VBScript, or WMI errors keep the guarded installation stopped before DiskPart. Close Setup and restart after correcting the disk or configuration. This does not modify supplied XML or set general USB write protection.

The **Generated Windows Setup** section is used for generated/exported XML, including scripts selected without a supplied `Autounattend.xml`. When an XML is supplied, the app preserves that file's Windows Setup settings instead. **Target disk** is the internal disk number passed to DiskPart and Windows Setup. **Install partition** identifies the Windows image destination; the standard generated layout uses partition `3`. **EFI MB** and **MSR MB** set the EFI System and Microsoft Reserved partition sizes. **Shrink MB** is the amount reserved for the Recovery partition. The EFI, Windows, and Recovery label fields name those volumes; the EFI/Win/Rec letter fields provide temporary Setup drive letters. Select **Edition** from its list; it must match an available image name in the selected ISO. **Prompt before erasing/installing** shows the **Ready to Reimage** Yes/No dialog before internal-disk reimaging, with **No** selected by default. Choose **OOBE language** and **OOBE keyboard** from the drop-downs to set the Windows locale and input profile after Setup; the defaults are English (United States) and US keyboard. The chosen language must be present in the selected Windows ISO. These settings bypass the language and keyboard prompts. The settings visibly dim while locked. **Generate Autounattend.xml** is always available: it asks where to save a standalone answer file using the currently displayed values, without changing them. The generated file writes one DiskPart command per Windows Setup step so shell quoting cannot combine commands. Labels support letters, numbers, spaces, hyphens, and underscores; temporary drive letters must be one letter. **Allow unsigned drivers** applies only to DISM `/ForceUnsigned` and cannot override Windows or Secure Boot signature enforcement.
...
12. Review the completion message. It reports successful and failed drives and gives the build-log path. When cache exists, choose **Keep cache** (default) to make the next matching build faster, or **Clear cache** to free local space. The choice appears after the entire queue finishes, never between partition writes. Keeping cache preserves it across app restarts; clearing may defer locked remnants until exit. USB files, source files, logs, and profiles remain intact.

The upper Activity progress bar displays only the current operation and byte-based completion percentage during transfers. The lower queue tracker begins empty, creates one equal-width segment per selected USB, and fills each segment progressively as its drive is partitioned, copied, and verified. Active progress uses uninterrupted repeating diagonal barber-pole bands on the current-operation bar, active queue fill, and writing drive card; the stripes are never drawn as isolated blocks. The animation is removed as soon as the queue finishes: success becomes static solid dark green and failure or cancellation becomes static red. ETA and total-queue time estimates are intentionally omitted because formatting, ISO mounting, small-file overhead, antivirus scanning, and changing USB speeds made them unreliable.
...
After DiskPart `clean`, the app refreshes Windows storage state and verifies that the selected drive has no partitions before continuing. An empty MBR disk does not need a redundant conversion. Storage errors serialized by PowerShell as CLIXML are decoded into readable build-log messages.
...
Windows media is staged and cached under `%LOCALAPPDATA%\LaptopQAUsbBuilder`. Driver sources are copied into the active build's private staging folder before DISM starts, so cache cleanup or a second app instance cannot remove files that servicing is using. The staging copy hashes same-size candidates, represents byte-identical payloads with hard links while preserving every path, and suppresses redundant INF entries only when their complete package directories are byte-identical. The build log reports linked file count, temporary bytes saved, and suppressed INF count. Driver staging overlaps the ISO copy; exact-duplicate hashing uses up to four workers, package preflight uses up to six workers, and ZIP/CAB extraction uses at most two concurrent workers. DISM operations against the mounted WIM remain serialized. Each DISM operation displays elapsed time and a real percentage when DISM supplies one. The app samples DISM CPU, process I/O, watched paths, and output; after five quiet seconds it confirms DISM is still working, and after ten uninterrupted inactive minutes it records a non-blocking warning while continuing to wait. If preparation completes but USB partitioning, copying, or verification later fails, all completed media and supporting driver caches are retained across app closure for the next retry; the completion dialog offers Keep cache for faster future builds or Clear cache to reclaim space. Selected source folders are never modified or reorganized, and newly added folders or packages are included automatically on the next recursive scan. Config's saved **Windows image compression** dropdown selects `FAST` (fast-compressed final `install.wim`), `MAX` (post-servicing maximum-compression final `install.wim`), or `ESD` (post-servicing recovery-compressed final `install.esd`, the default). Driver injection always occurs against a mountable working WIM. MAX and ESD perform their final compacting export after the driver commit; ESD removes the working WIM after creating `install.esd`. A matching later build reuses the cache. Cache identity includes the source ISO hash, edition, driver-folder manifest, unsigned-driver choice, compression mode, and staging algorithm version. Use Config's **Clear Cache** action to remove cached or incomplete work; its post-exit pass handles app-locked remnants. Ensure the system drive has ample free space before first-time preparation.

The segmented total-progress tracker includes shared preparation instead of waiting for USB writes. For Windows-media builds, validation, driver/archive preparation, ISO hashing and staging, WIM export, driver injection, and commit advance the first 55 percent of every selected-drive segment together. The remaining 45 percent of each segment advances independently while that USB is partitioned, copied, and verified. The current-activity bar continues to restart for each individual operation. Active fills use an animated diagonal stripe brush on both progress trackers.
...
Configuration displays the combined size of `MediaCache`, `DriverPackCache`, `DriverPayloadCache`, and incomplete `Staging` work. **Clear Cache** performs a file-by-file best-effort deletion immediately while preserving logs and settings, records a pending-cleanup marker, and launches a hidden post-exit cleanup helper for anything that remained locked when all USB Drive Builder windows have closed. A named cross-instance guard prevents the immediate pass from racing active media preparation.

The main window starts at 1180 × 700 DIPs or 32 DIPs inside the current Windows work area, whichever is smaller, with a 760 × 480 minimum and a standard maximize/restore control. Its fixed 1360 × 800 logical surface scales uniformly as the window is resized. Configuration starts at 820 × 733 or 64 DIPs inside the work area, whichever is smaller, has a 680 × 520 minimum, and uniformly scales its 940 × 840 logical surface. Build cancellation is exposed during preflight as well as USB writing. The cancellation token reaches ISO hashing/copying, archive expansion, driver staging and validation, DISM monitoring, PowerShell storage operations, and USB file copies. Active DISM is terminated, then cleanup uses a non-cancellable DISM discard so a mounted WIM is not stranded; ISO dismount also ignores the already-cancelled build token. The unique staging tree is deleted in `PrepareAsync` cleanup.
