...
The prepared media writer records the WIM-reported edition ID in `sources\ei.cfg` with Retail channel metadata and no embedded key. That media-level file suppresses the Windows Setup product-key page. The generated `Autounattend.xml` intentionally has no `ProductKey` element; a standalone generated XML placed on third-party media cannot create `ei.cfg` by itself.
