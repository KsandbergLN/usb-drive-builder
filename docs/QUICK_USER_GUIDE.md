...
Configuration reports the total build-cache size. Use **Clear Cache** to remove cached Windows media and temporary driver data after confirmation; build logs and saved settings are retained. Cache clearing is blocked while another USB Drive Builder instance is preparing media.
...
The app immediately shows **Preparing build** with a determinate current-activity progress bar while it checks the selected disks, source paths, capacity, and prepares or retrieves cached Windows media. DISM stages display elapsed time and use a real percentage whenever DISM reports one. Quiet periods explicitly say DISM is still working; ten minutes without detected CPU, I/O, file, or output activity produces a warning but does not stop servicing. During a queued build, a second tracker begins empty and divides evenly into one segment per selected USB. Shared Windows-media preparation advances every selected-drive segment together through validation, extraction, hashing, staging, export, injection, and commit. USB partitioning, copying, and verification then advance only the segment belonging to the active drive. The selected drive card is pale green; while writing it is dark green with moving diagonal green stripes, succeeds as solid dark green, and fails red. If prepared Windows media is followed by a USB partitioning, copying, or verification failure, the app retains its media and driver caches after close so the next retry can reuse them. Successful completion restores normal cache cleanup. If preflight succeeds, it proceeds directly into the USB queue without another confirmation or driver-report prompt. Safety and preparation failures still stop before anything is erased.
