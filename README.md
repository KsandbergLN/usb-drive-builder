...
The segmented total-progress bar includes shared preparation: Windows-media validation, extraction, hashing, staging, export, driver injection, and commit advance every selected-drive segment together. Once USB writing begins, only the active drive's segment advances through partitioning, copying, and verification. The current-activity bar remains specific to the current operation and restarts for each new stage.

Configuration shows the combined size of the Windows media and temporary driver caches. **Clear Cache** removes those cache folders after confirmation while retaining build logs and saved settings, and it refuses to run while another app instance is preparing media.
