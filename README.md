...
The **Cache location** field controls where the app stores media preparation, driver-pack extraction, driver-payload expansion, and temporary staging data. It defaults to `C:\Cache`; choose another absolute folder in Config when a different local drive has more free space. Application logs remain under `%LOCALAPPDATA%\LaptopQAUsbBuilder\Logs`.
